(() => {
  ...
  const backIconMarkup=()=>'<img class="back-icon" src="/assets/'+(Number(me?.gold)>0?'back-gold.png':'back-silver.png')+'" alt="Orqaga">';
  const usernameDialog=document.createElement('dialog');usernameDialog.id='usernameDetail';usernameDialog.innerHTML='<form method="dialog" id="usernameDetailForm"><button type="button" id="usernameBack" class="username-back">'+backIconMarkup()+'</button><h2>Username</h2><div class="username-detail-card"><b>Username qo‘yish</b><div class="username-detail-input"><span>t.me/</span><input id="usernameDetailInput" autocomplete="off" autocapitalize="none" spellcheck="false"></div></div><p class="username-detail-help">Bu username orqali odamlar sizni topishi va siz bilan bog‘lanishi mumkin.</p><div class="row"><button type="button" id="usernameCancel" class="secondary">Bekor qilish</button><button type="button" id="usernameApply">Tayyor</button></div></form>';
  ...
  const backIconStyle=document.createElement('style');backIconStyle.textContent='.back-icon{width:24px;height:24px;object-fit:contain;display:block}.username-back .back-icon{width:28px;height:28px}';document.head.append(backIconStyle);
  ...
  const openBlankUsernameWindow = () => {
    ...
    if(!win){
      ...
      if(!blank){
        ...
        rows.addEventListener('drop',e=>{e.preventDefault();const target=e.target.closest('.tg-row');if(!dragged||!target||dragged===target)return;target.before(dragged);renderRows()});
        blank.querySelector('.order-save').onclick=async()=>{
          const button=blank.querySelector('.order-save');
          if(button.disabled)return;
          const values=[...rows.querySelectorAll('.tg-row')].map(x=>x.dataset.username);
          button.disabled=true;
          try {
            const fresh=await api('profile',{name:me.name,app_username:values[0]||'',usernames:values,bio:me.bio||''});
            me={...me,...fresh};
            // The parent form must submit the same order, not its old draft.
            $('editUsername').value=me.usernames.map(x=>'@'+x).join('\n');
            updateUsernameList();
            renderProfile();
            blank.close();
        ...
        blank.querySelector('form').onsubmit=e=>{e.preventDefault();blank.querySelector('.order-save').click()};
        blank.refreshRows=()=>{
          const values=dialog.open ? $('editUsername').value.split(/\s+/).map(x=>x.replace(/^@/,'').toLowerCase()).filter(Boolean) : (me.usernames||(me.app_username?[me.app_username]:[]));
          rows.replaceChildren(...values.map(value=>{const row=document.createElement('div');row.className='tg-row';row.dataset.username=value;return row}));
          renderRows();
        };
      const s=document.createElement('style');
        ...
        s.textContent+='#blankUsernameWindow .tg-row .order-up,#blankUsernameWindow .tg-row .order-down{flex:0 0 28px;width:28px;min-width:28px;height:28px;min-height:28px;margin:0;padding:0;display:grid;place-items:center;border:1px solid #536b80;border-radius:7px;background:#263b4d;color:#eef6ff;font:18px/1 Arial;box-shadow:none}#blankUsernameWindow .tg-row .order-up{margin-right:-6px}#blankUsernameWindow .tg-row button:disabled{opacity:.3;cursor:default}';
        s.textContent+=`
          #blankUsernameWindow .tg-row{background:#293c4d}
          #blankUsernameWindow .tg-icon{background:#347897;box-shadow:none}
          #blankUsernameWindow .tg-row .order-up,
          #blankUsernameWindow .tg-row .order-down{
            background:#38546b;border-color:#506b80;
            box-shadow:inset 0 1px 0 #ffffff18,0 3px 0 #142330,0 4px 5px #0004;
            transition:transform .12s,box-shadow .12s;
          }
          #blankUsernameWindow .tg-row button:active:not(:disabled){
            transform:translateY(2px);box-shadow:inset 0 1px 2px #0003,0 1px 0 #142330;
          }
          #blankUsernameWindow .tg-row button:disabled{box-shadow:none}
          #blankUsernameWindow .order-save{
            background:#347897;border:1px solid #52869e;
            box-shadow:inset 0 1px 0 #ffffff18,0 4px 0 #1b4358,0 6px 8px #0004;
          }
          #blankUsernameWindow .order-save:active:not(:disabled){transform:translateY(3px);box-shadow:0 1px 0 #1b4358}
          #blankUsernameWindow .blank-window-close{
            display:grid;place-items:center;min-width:42px;min-height:42px;
            padding:0;margin:0;box-sizing:border-box;font:28px/1 Arial,sans-serif;
            background:#304657;box-shadow:inset 0 1px 0 #ffffff12,0 3px 0 #142330,0 5px 7px #0004;
          }
        `;
    ...
    try{
      ...
      win.document.body.innerHTML='<main class="tg-usernames"><header><button id="cancel"><img class="back-icon" src="/assets/'+(Number(me?.gold)>0?'back-gold.png':'back-silver.png')+'" alt="Orqaga"></button><h1>Username</h1><button id="save">✓</button></header><section class="tg-card tg-primary"><b>Username qo‘yish</b><div class="tg-input"><span>t.me/</span><textarea id="usernames" rows="1">'+current+'</textarea></div></section><p class="tg-help">Odamlar sizni username orqali topishi va bog‘lanishi mumkin.</p><section class="tg-card"><b>Username tartibi</b><div id="usernameList"></div></section><p id="error"></p></main>';
      ...
      win.document.head.insertAdjacentHTML('beforeend','<style>.back-icon{width:28px;height:28px;object-fit:contain;display:block}</style>');
