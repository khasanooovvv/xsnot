(() => {
  ...
  const updateUsernameList = () => { const list=$('usernameList'); if(!list)return; const items=$('editUsername').value.split(/\s+/).map(x=>x.replace(/^@/,'')).filter(Boolean); list.innerHTML=items.map((x,i)=>`<div class="username-list-row"><span class="username-link-icon">↗</span><span><b>@${x}</b><small>${i?'Qo‘shimcha username':'Asosiy username'}</small></span><span class="username-drag">☷</span></div>`).join(''); };
