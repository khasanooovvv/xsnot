(() => {
  ...
  dialog.innerHTML = `<form id="profileEditForm">
    ...
    <section class="username-settings" aria-labelledby="usernameSettingsTitle"><h3 id="usernameSettingsTitle">Username</h3><div class="username-entry"><span>t.me/</span><textarea id="editUsername" rows="3" placeholder="username&#10;qo‘shimcha_username" autocomplete="off" autocapitalize="none" spellcheck="false" aria-describedby="usernameHint"></textarea></div><small class="username-help">Username orqali odamlar sizni topishi va bog‘lanishi mumkin.</small><h4>Username’lar tartibi</h4><div id="usernameList" class="username-list"></div><button id="addUsername" type="button" class="secondary">+ Username qo‘shish</button><small id="usernameHint">Oddiy profil: 1 ta, Silver: 2 ta, Gold: 3 ta username.</small></section>
  ...
  const usernameStyle=document.createElement('style');usernameStyle.textContent='.username-entry{display:none!important}.username-list-row{display:flex;align-items:center;gap:10px;width:100%;padding:12px;text-align:left;border:1px solid var(--border);border-radius:0;color:var(--text)!important;background:var(--surface)!important}.username-list-row span:nth-child(2){min-width:0;flex:1}.username-list-row:hover{background:var(--surface-2)!important}#usernameDetail{width:min(94vw,440px);padding:20px;border:1px solid var(--border);border-radius:24px;background:var(--surface);color:var(--text)}#usernameDetail::backdrop{background:#0008}.username-back{width:42px!important;padding:5px!important;background:transparent!important;color:var(--text)!important;font-size:28px!important}.username-detail-card{margin-top:12px;padding:16px;border-radius:18px;background:var(--surface-2)}.username-detail-card>b{display:block;color:var(--accent);margin-bottom:12px}.username-detail-input{display:flex;align-items:center;gap:4px;padding:12px;border-radius:14px;background:var(--surface);border:1px solid var(--border);color:var(--muted)}.username-detail-input input{margin:0!important;padding:0!important;border:0!important;background:transparent!important;color:var(--text)!important}.username-detail-help{color:var(--muted);line-height:1.5}';document.head.append(usernameStyle);
  const updateBioCount = () => { $('bioCount').textContent = `${$('editBio').value.length} / 300`; };
  const updateUsernameList = () => { const list=$('usernameList'); if(!list)return; const items=$('editUsername').value.split(/\s+/).map(x=>x.replace(/^@/,'')).filter(Boolean); list.innerHTML=items.map((x,i)=>`<button type="button" class="username-list-row" data-username="${x}"><span class="username-link-icon">↗</span><span><b>@${x}</b><small>${i?'Qo‘shimcha username':'Asosiy username'}</small></span><span class="username-drag">☷</span></button>`).join(''); };
