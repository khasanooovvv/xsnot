(() => {
  ...
  function confirmDelete(text){return new Promise(resolve=>{dialog.replaceChildren();dialog.className='dm-confirm';const p=document.createElement('p');p.textContent=text;const row=document.createElement('div');row.className='dm-confirm-actions';const cancel=document.createElement('button');cancel.type='button';cancel.textContent='Bekor qilish';const ok=document.createElement('button');ok.type='button';ok.textContent='O‘chirish';const finish=value=>{dialog.close();dialog.className='';resolve(value)};cancel.onclick=()=>finish(false);ok.onclick=()=>finish(true);row.append(cancel,ok);dialog.append(p,row);dialog.showModal()})}
  async function hide(id){if(!await confirmDelete('Chat sizning ro‘yxatingizdan o‘chirilsinmi?'))return;await request('/chats/'+id,'DELETE');if(current?.id===id)el('dmBack').click();await refreshList()}
