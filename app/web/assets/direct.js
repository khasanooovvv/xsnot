(() => {
  ...
  function messageMenu(m){const id=current?.id;if(!id)return;const name=current?.partner?.name||'suhbatdosh';actions([['Tahrirlash',()=>{editing=m.id;el('dmInput').value=m.text;el('dmEditBar').hidden=false;el('dmInput').focus()}],['O‘chirish',async()=>{const scope=await confirmChoice('Xabarni o‘chirish',`Mendan va ${name}dan o‘chirish`,`Faqat mendan o‘chirish`);if(!scope)return;await request(`/chats/${id}/messages/${m.id}?scope=${scope}`,'DELETE');if(current?.id===id){if(editing===m.id)cancelEdit();await poll()}}]])}
