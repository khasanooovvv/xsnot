        ...
        if connection.dialect.name == "postgresql":
            ...
            await connection.execute(text("""
                ...
                BEGIN
                    ...
                    END IF;
                    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='direct_messages' AND column_name='created_at')
                       AND NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='direct_messages' AND column_name='updated_at') THEN
                        ALTER TABLE direct_messages ADD COLUMN updated_at TIMESTAMPTZ;
                        UPDATE direct_messages SET updated_at = created_at WHERE updated_at IS NULL;
                    END IF;
                END $$;
            """))
            ...
            await connection.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS gold_hidden_username VARCHAR(24)"))
            await connection.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS gold_hidden_username_until TIMESTAMPTZ"))
            ...
            await connection.execute(text("ALTER TABLE user_usernames ADD COLUMN IF NOT EXISTS hidden_until TIMESTAMPTZ"))
            # Username search is a case-insensitive substring search.  The
            # regular btree indexes above cannot accelerate LIKE '%term%'.
            # Trigram indexes do, including terms containing '_' or digits.
            await connection.execute(text("CREATE EXTENSION IF NOT EXISTS pg_trgm"))
            await connection.execute(text("CREATE INDEX IF NOT EXISTS ix_users_app_username_trgm ON users USING gin (lower(app_username) gin_trgm_ops)"))
            await connection.execute(text("CREATE INDEX IF NOT EXISTS ix_user_usernames_username_trgm ON user_usernames USING gin (lower(username) gin_trgm_ops)"))
