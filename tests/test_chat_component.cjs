...
const viewport=fs.readFileSync(path.join(__dirname,'..','app','web','assets','chat-viewport.css'),'utf8');
...
for(const token of ['direct-fullscreen','chat-shared-avatar','directBack'])if(!direct.includes(token))throw Error('direct runtime parity token missing: '+token);
for(const token of ['position:fixed','height:100dvh','chat-shared-messages'])if(!viewport.includes(token))throw Error('fullscreen viewport rule missing: '+token);
